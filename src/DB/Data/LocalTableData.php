...
class LocalTableData
{
  ...
  public function getOldNewDiff($table, $key): array
  {
    $diffSequence = [];
...
//        $this->source->set(PDO::FETCH_NAMED);
...
//        $this->source->setFetchMode(PDO::FETCH_ASSOC);
    ...
    foreach ($result1 as $row) {
      $diffSequence[] = new InsertData($table, [
        'keys' => array_only((array)$row, $key),
        'diff' => new DiffOpAdd(array_except((array)$row, '_connection')),
      ]);
    }
    foreach ($result2 as $row) {
      $diffSequence[] = new DeleteData($table, [
        'keys' => array_only((array)$row, $key),
        'diff' => new DiffOpRemove(array_except((array)$row, '_connection')),
      ]);
    }
  ...
  public function getChangeDiff($table, $key): array
  {
        ...
        if (starts_with($k, 's_')) {
          ...
          if (isset($row[$targetKey])) {
            ...
            if ($sourceValue != $targetValue) {
              $diff[$theKey] = new DiffOpChange($targetValue, $sourceValue);
            }
          } else {
            $diff[$theKey] = new DiffOpChange(null, $sourceValue);
          }
        }
      }
